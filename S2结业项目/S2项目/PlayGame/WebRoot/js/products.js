jQuery(function($) {
	...
	function output_cookie()
	{
		...
		for ( var x in cookie )
		{
			...
			var $new = $('<tr class="prs">\
							<td> \
								<input type="hidden" class="sl_id" value="'+cookie[x].id+'"/> \
							...
							<td> \
								<div class="qty-btn-group"> \
									...
									<input class="prsnum" type="text" value="' + cookie[x].qty + '" /> \
									<button type="button" class="up"><i class="iconfont-caret-up inline-middle"></i></button> \
								</div> \
							</td> \
							<td class="hidden-xs"><strong class="text-bold row-total">$' + temp*cookie[x].qty + '</strong></td> \
	 ...
	 * |
	 * ---------------------------------------------------------------------------------- |
	 * jQuery Range slider - noUiSlider | Git:
	 * https://github.com/leongersen/noUiSlider | URL:
